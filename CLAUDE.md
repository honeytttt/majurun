...
- Next build on this branch: **build 144** (version 1.0.0+144)
...
**Pre-existing tracked files with credentials (already public — cannot un-expose):**
- `GoogleService-Info.plist` — was committed before gitignore rule was added
- `ios/Runner/GoogleService-Info.plist` — same
- Mitigation: Firebase App Check is enabled; restrict API keys in Firebase Console to your app's bundle ID
