...
**Current TestFlight version:** `1.0.5+253` — 0→5K banner now actually starts the plan (was opening a dead-end overview) + app download link added to post/video shares + run-share link moved up (below stats, not buried at bottom). (252 = stats rebuild fix, 250/251 banner+stats, 249 default-RUN-tab + share-link, etc.) (249 default-RUN-tab + share-link, 248 run post-text, 247 double-submit, 246 START, 245 audio, 244 freeze all included.)
