...
- ✅ **VERIFIED Jul 26, 2026 — the Crashlytics "Missing dSYM" list LIES; do not chase it.** The console listed UUID `37E3E841-822C-30DA-B5EB-CC1F4F375DBB` as **Missing (required)** for 1.0.5 (255). That exact UUID was uploaded successfully by build 255's own CI run on Jul 18 (`Successfully submitted symbols ... UUID 37e3e841822c30dab5ebcc1f4f375dbb in dSYM: .../Runner.app.dSYM`). The list is stale / the crash arrived before symbol processing finished. **No CI change needed.** Only **1.0.4 (249)** is genuinely missing (pre-dates the dSYM step) — permanently unsymbolicated, nothing to do. The many "Missing (optional)" rows are header-only interop shims (`FirebaseAuthInterop`, `RecaptchaInterop`, …) that carry no symbols by design — ignore them too.
- 📡 **Sentry CONFIRMED CAPTURING (Jul 26, 2026)** — first real events arrived, proving the build-254 DSN fix + build-256 handler chaining work end-to-end. We are no longer blind. First two issues: `FLUTTER-1` **ApplicationNotResponding (Fatal ANR)** at `com.majurun.app.MainActivity in configureFlutterEngine` — a main-thread block during Flutter engine setup, i.e. the STARTUP-HANG family (same family as the build-244 freeze); and `FLUTTER-2` `HTTPClientError` 500 on iOS (`-[__NSCFLocalSessionTask _task_onqueue_didFinish]`). **Check FLUTTER-2's request URL — if it is `verifySubscription`, it is a purchase-flow failure and takes priority**, because build 257 swapped the native billing library and no sandbox purchase was ever tested.
