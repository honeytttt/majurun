...
**Current TestFlight version:** `1.0.5+254` — treadmill TIME entry + crash-recovery now SAVES interrupted runs (was inert/discarded) + Sentry turned ON (DSN was never injected → crash reporting was silently dead) + dynamic release version. (253 = 0→5K banner starts plan + app link in post/run shares; 252 stats rebuild; 249 default-RUN-tab + share-link.) ⚠️ DEVICE-TEST crash-recovery (kill app mid-run → relaunch → Save) before trusting it. (249 default-RUN-tab + share-link, 248 run post-text, 247 double-submit, 246 START, 245 audio, 244 freeze all included.)
