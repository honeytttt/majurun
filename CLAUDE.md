...
- 🔴 **CORRECTED Jul 29, 2026 — iOS crash visibility IS currently BROKEN. The earlier "the Missing dSYM list LIES, do not chase it" note was WRONG and has been deleted.** What is true: CI genuinely uploads the symbols. What is false: the conclusion that there was therefore nothing to fix. **iOS Crashlytics is holding 4 unprocessed crashes and shows a misleading 100% crash-free / "No issues found"** — unprocessed events are not symbolicated, so they never become issues AND never count against the crash-free rate. Do not read 100% on iOS as healthy while that banner is present.
  ...
  - **Leading hypothesis to check first: app-ID mismatch.** `upload-symbols -gsp` routes symbols by the `GOOGLE_APP_ID` inside the plist. If the `GOOGLE_SERVICE_INFO_PLIST_PROD` secret holds a plist for a different/older iOS app entry than the one receiving crashes, every upload succeeds into a ghost app and the receiving app stays permanently "missing". **Verify the secret's `GOOGLE_APP_ID` equals the iOS app ID under Firebase → Project settings → Your apps.** Second hypothesis: Firebase-side processing lag/bug (but 257's symbols went up Jul 25 and were still "missing" on Jul 29 — too long to be lag).
