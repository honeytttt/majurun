...
**Sentry is clean, but a clean Sentry at ~16 installs is weak evidence.** All 3 issues were from ONE device each and two were not real users at all. Absence of crash reports at this scale does not predict stability at 10× the users. The real crash-rate sources are Play Console **Android vitals** (Play-installed users only — excludes the sideloaded-emulator noise that produced FLUTTER-1) and **Crashlytics crash-free rate for 257**. ⚠️ **The last recorded Crashlytics baseline is build 237 at ~85% crash-free on Android, which is poor** — that number has NOT been re-checked against 257. Re-check it before promoting; if it is still ~85%, promotion amplifies a known-bad rate.
