...
**Current TestFlight version:** `1.0.5+252` — run home page: free "0 → 5K" beginner-plan banner + fixed lifetime stats showing 0 until first run (252 = real rebuild fix: 250/251's initState refresh updated values but never notified RunController's listeners, so the Consumer never rebuilt — added notifyListeners() in RunController.refreshHistoryStats). (249 default-RUN-tab + share-link, 248 run post-text, 247 double-submit, 246 START, 245 audio, 244 freeze all included.)
